...
let HassRssCardEditor = class HassRssCardEditor extends i$1 {
        ...
        ${feeds.map((feed, index) => b `
            <div class="feed-row">
              ...
              <ha-textfield
                ...
                .value=${feed.category ?? ''}
                @input=${(ev) => this._updateFeed(index, 'category', ev.target.value)}
              ></ha-textfield>
              ${feeds.length > 1
            ? b `
                    <ha-icon-button
    ...
    _updateConfig(key, value) {
        ...
        if (key === 'feeds' && Array.isArray(value)) {
            nextValue = value.filter((feed) => typeof feed === 'object' &&
                feed &&
                'entity' in feed &&
...
let HassRssCard = class HassRssCard extends i$1 {
    ...
    _renderHeader(features) {
        if (!features.show_refresh_button)
            return A;
        return b `
      <div class="header">
    ...
    _renderEmpty() {
        const hasMissing = (this._config.feeds ?? []).some((f) => !this.hass.states[f.entity]);
        if (hasMissing) {
            return b `<div class="error">One or more feed entities are unavailable.</div>`;
        }
    ...
    _renderList(items, features) {
      ...
      <div class="list-items">
        ${items.map((item) => b `
            ...
            : A}
                ${features.show_relative_time
            ? b `<div class="meta">${formatRelativeTime(item.published, this.hass.locale?.language)}</div>`
            : A}
              </div>
            </div>
          `)}
...
window.customCards.push({
    ...
    preview: true,
    documentationURL: 'https://github.com/your-org/hass-rss-card#readme',
});
